...
class Replayset(torch.utils.data.Dataset):
    ...
    def FDA_source_to_target(self, src_img, trg_img_amp, L=0.1):
    # exchange magnitude
    # input: src_img, trg_img

    # get fft of both source and target
        fft_src = self.rfft( src_img.clone()) 
        ...
        src_in_trg = self.irfft( fft_src_ )

        return src_in_trg.real

    def __init__(self, path, labels_old, labels, num_per_class=2, fda=False, transform=None):
        # import os
        ...
        print("Using FDA setting is set to: "+str(self.fda))
    ...
    def __getitem__(self, index):
        ...
        if self.fda:
            img = self.FDA_source_to_target(img.unsqueeze(0), self.psc_real_t, L=0.1).squeeze(0)
